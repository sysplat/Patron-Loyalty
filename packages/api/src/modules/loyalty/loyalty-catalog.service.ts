...
export class LoyaltyCatalogService {
  ...
  async redeemReward(orgId: string, customerId: string, rewardId: string) {
    ...
    const redemption = await this.prisma.withTenant(orgId, async (tx) => {
      ...
      await this.accounts.burnPoints(orgId, account.id, reward.pointsCost, {
        sourceType: 'reward',
        sourceId: rewardId,
  ...
  async redeemCoupon(orgId: string, code: string, accountId: string) {
    await this.validateCoupon(orgId, code, accountId);
    return this.prisma.withTenant(orgId, async (tx) => {
      const coupon = await tx.loyaltyCoupon.findFirstOrThrow({
        where: { orgId, code: code.toUpperCase() },
      });
      await tx.loyaltyCoupon.update({
        where: { id: coupon.id },
