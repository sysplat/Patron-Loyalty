...
ARG NEXT_PUBLIC_WEB_URL=http://localhost:3003
ARG NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=
ARG NEXT_PUBLIC_SENTRY_DSN=
ARG SENTRY_ADMIN_DSN=
...
ARG SENTRY_REQUIRE_SOURCE_MAPS=0
...
ENV SENTRY_ADMIN_DSN=${SENTRY_ADMIN_DSN}
ENV SENTRY_AUTH_TOKEN=${SENTRY_AUTH_TOKEN}
ENV SENTRY_ORG=${SENTRY_ORG}
ENV SENTRY_PROJECT=${SENTRY_PROJECT_ADMIN}
ENV SENTRY_PROJECT_ADMIN=${SENTRY_PROJECT_ADMIN}
ENV SENTRY_RELEASE=${SENTRY_RELEASE}
ENV RAILWAY_GIT_COMMIT_SHA=${RAILWAY_GIT_COMMIT_SHA}
ENV SENTRY_REQUIRE_SOURCE_MAPS=${SENTRY_REQUIRE_SOURCE_MAPS}
ENV NEXT_TELEMETRY_DISABLED=1

COPY package.json pnpm-lock.yaml tsconfig.base.json ./
COPY railway/docker/pnpm-workspace.admin.yaml pnpm-workspace.yaml
COPY apps/admin/package.json apps/admin/
COPY packages/shared/package.json packages/shared/
COPY packages/frontend-core/package.json packages/frontend-core/

RUN pnpm install --frozen-lockfile --filter "@queueplatform/admin..."
